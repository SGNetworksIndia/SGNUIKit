...
window.escapeHTML = window.html_entities_encode = window.html_special_chars_encode = (str) => {
  const escapeChars = {
    "¢": "cent",
    "£": "pound",
    "¥": "yen",
...
window.unescapeHTML = window.html_entities_decode = window.html_special_chars_decode = (str) => {
  const htmlEntities = {
    nbsp: " ",
    cent: "¢",
    pound: "£",
  ...
  return str.replace(/&([^;]+);/g, function(entity, entityCode) {
    ...
    } else if(/^#x([\da-fA-F]+)$/.test(entityCode)) {
      const match = entityCode.match(/^#x([\da-fA-F]+)$/);
      return String.fromCharCode(parseInt(match[1], 16));
      /*eslint no-cond-assign: 0*/
    } else if(/^#(\d+)$/.test(entityCode)) {
      const match = entityCode.match(/^#(\d+)$/);
